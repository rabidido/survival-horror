...
export class UI {
  constructor(game) {
    this.game = game;
    this.el = {
      hud: q('#hud'), touch: q('#touch'), overlay: q('#overlay'),
  ...
  showHUD(on) {
    ...
    this.el.touch.classList.toggle('hidden', !(on && this.game.touchMode));
  }
  ...
  setPrompt(text, key) {
    ...
    this.el.promptKey.textContent = key || (this.game.touchMode ? 'ACT' : 'E');
    ...
    const render = () => {
      ...
      if (t === 'items') {
        ...
        for (let i = 0; i < g.inv.size; i++) {
          ...
          slots += `<div class="slot${sel}${eq}" data-i="${i}">` +
        ...
        body = g.files.length
          ? `<div class="files">${g.files.map((f, i) =>
            `<button class="file" data-f="${i}">${ITEMS[f].title || ITEMS[f].name}</button>`).join('')}</div>`
  ...
  openItemBox(game) {
    const render = () => {
      ...
      this.openOverlay('box', `<div class="panel">
        ...
        <div class="panel-body">
          <p class="khint">Tap an item to move it between your pack and the trunk.</p>
